...
abstract class GradleBuildRootsLocator {
    ...
    fun findScriptBuildRoot(filePath: String, searchNearestLegacy: Boolean = true): ScriptUnderRoot? {
        ...
        scriptInfo?.buildRoot?.let { imported -> return ScriptUnderRoot(filePath, imported, scriptInfo) }
        ...
        roots.getStandaloneScriptRoot(filePath)?.let { return ScriptUnderRoot(filePath, it, standalone = true) }
        ...
        if (filePath.endsWith("/build.gradle.kts") ||
            ...
            roots.getBuildByProjectDir(filePath.substringBeforeLast("/"))?.let { return ScriptUnderRoot(filePath, it) }
        }
